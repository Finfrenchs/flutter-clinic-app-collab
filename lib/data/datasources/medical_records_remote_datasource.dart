...
class MedicalRecordsRemoteDatasource {
  ...
  Future<Either<String, GetMedicalRecordsByPatientScheduleIdResponseModel>>
      getMRByPatientScheduleId(int id) async {
    final authData = await AuthLocalDataSource().getAuthData();
    final url =
        Uri.parse('${Variables.baseUrl}/api/api-medical-records/services/$id');
    final response = await http.get(
      url,
      headers: {
        'Authorization': 'Bearer ${authData?.token}',
    ...
    if (response.statusCode == 200) {
      return Right(GetMedicalRecordsByPatientScheduleIdResponseModel.fromJson(
          response.body));
    } else {
      return const Left('Gagal mendapatkan data item layanan pasien');
    }
  }
}
